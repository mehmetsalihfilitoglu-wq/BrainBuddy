...
/**
 * Isolated per-user Daily Challenge state store. Deliberately a SEPARATE database from brainbuddy.db so
 * the content DB's schema/version is never touched (no migration risk). Content questions still live in
 * brainbuddy.db and are read via QuestionDao; only user progress lives here.
 */
@Database(
    entities = [
