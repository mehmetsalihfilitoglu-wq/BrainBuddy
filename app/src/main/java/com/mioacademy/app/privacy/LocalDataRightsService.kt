package com.mioacademy.app.privacy

import android.content.Context
import com.mioacademy.app.auth.AuthProvider
import com.mioacademy.app.core.ProfileStore
import com.mioacademy.app.sync.PrefsSnapshotCodec
import com.mioacademy.app.sync.SyncRegistry
...
class LocalDataRightsService(context: Context) : DataRightsService {
    ...
    override suspend fun deleteAccount(userId: String?): DeletionResult {
        return try {
            runCatching { AuthProvider.repository(ctx).deleteAccount() }
            when (val local = deleteLocalData()) {
                is DeletionResult.Error -> local
                else -> DeletionResult.LocalDoneCloudPending // cloud erase runs server-side later
            }
        } catch (e: Exception) {
