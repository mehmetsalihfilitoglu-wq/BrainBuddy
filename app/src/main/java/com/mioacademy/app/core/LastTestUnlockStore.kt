...
/** One-shot unlock token for "Last Tests" replay. Consumed when QuizActivity reads it. */
class LastTestUnlockStore(context: Context) {
    private val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)

    fun createUnlock(quizId: String, questionIds: List<String>): String {
    ...
    /** Returns questionIds if token valid for quizId, then consumes. */
    fun consumeUnlock(token: String, quizId: String): List<String>? {
