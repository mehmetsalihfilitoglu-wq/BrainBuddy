...
import com.mioacademy.app.db.GradeSubjectDifficultyCount
import com.mioacademy.app.db.LgsCandidateRow
...
import com.mioacademy.app.db.QuotaSyntheticQuestions
import com.mioacademy.app.db.RoomQuizDataStore
import com.mioacademy.app.db.SyntheticHardQuestionGenerator
...
class QuestionRepository(private val context: Context) {
    ...
    private fun getGlobalPool(): List<Question> {
        ...
        val examStore = com.mioacademy.app.core.ExamPackStore(context)
    ...
    private fun parseQuestion(o: JSONObject): Question {
        ...
        val examType = try { com.mioacademy.app.quiz.ExamType.valueOf(examStr) } catch (_: Exception) { com.mioacademy.app.quiz.ExamType.GENERAL }
    ...
    fun recordAnswers(answers: List<AnswerRecord>, questionsMap: Map<String, Question>? = null, testId: String? = null) {
        val userId = com.mioacademy.app.core.ActiveProfileManager.getActiveProfileId(context)
    ...
    private suspend fun pickQuizQuestionsForLGSInternal(
        ...
        fun poolFor(subj: Subject): List<LgsCandidateRow> {
            return subjectPools.getOrPut(subj) {
                val dbKey = com.mioacademy.app.db.QuestionMapper.toDbSubject(subj)
        ...
        for ((subjEnum, preferredType) in slots) {
            if (selectedRows.count { com.mioacademy.app.db.QuestionMapper.mapSubject(it.subject) == subjEnum } >= (blueprint.subjectTargets[subjEnum] ?: 0)) continue
            ...
            val typeCountsInSubject = selectedRows.filter { com.mioacademy.app.db.QuestionMapper.mapSubject(it.subject) == subjEnum }
        ...
        val lgsPoolSummary = LGS_SUBJECTS.joinToString(" | ") { sk ->
            ...
            "$sk: pool=${subjectPools[subj]?.size ?: 0} selected=${selectedRows.count { com.mioacademy.app.db.QuestionMapper.mapSubject(it.subject) == subj }}"
        }
        ...
        lastSubjectCounts = blueprint.subjectTargets.keys.associate { subj ->
            com.mioacademy.app.db.QuestionMapper.toDbSubject(subj) to orderPreserved.count { it.subject == subj }
        }
    ...
    private fun balanceToAvoidConsecutiveSameType(rows: List<LgsCandidateRow>): List<LgsCandidateRow> {
        ...
        val bySubject = rows.groupBy { com.mioacademy.app.db.QuestionMapper.mapSubject(it.subject) }
    ...
    fun pickQuizQuestions(
        ...
        val picker = SpacedRepetitionPicker(
            historyDao = com.mioacademy.app.db.DatabaseProvider.get(context).historyDao(),
            roomStore = roomStore,
