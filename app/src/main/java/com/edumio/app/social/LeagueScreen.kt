...
import com.edumio.app.databinding.ActivityLeagueBinding
import com.edumio.app.league.LeagueEntry
import com.edumio.app.league.LeagueHelper
import com.edumio.app.league.LeagueScoring
import com.edumio.app.league.LeagueStore
import com.edumio.app.league.LeagueTier
...
class LeagueScreen : AppCompatActivity() {
    ...
    private fun bindPodiumAvatar(imgView: android.widget.ImageView, entry: LeagueEntry) {
        ...
        val mascotRes = if (!mascotId.isNullOrEmpty()) {
            com.edumio.app.avatar.AvatarCatalog.items()
                .find { it.id == mascotId }
                ?.previewDrawableRes
                ?.takeIf { it != 0 }
        } else null
...
class LeagueAdapter(
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        val mascotItem = if (!mascotId.isNullOrEmpty())
            com.edumio.app.avatar.AvatarCatalog.items().find { it.id == mascotId }
        else null
