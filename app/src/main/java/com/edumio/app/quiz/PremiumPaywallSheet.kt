...
import com.edumio.app.billing.BillingProvider
import com.edumio.app.billing.PlanOffer
import com.edumio.app.billing.PurchaseResult
import com.edumio.app.billing.SubscriptionPlan
import com.edumio.app.billing.SubscriptionState
 ...
 * subscription status, purchase, and restore — all through [com.edumio.app.billing.BillingRepository],
...
class PremiumPaywallSheet : BottomSheetDialogFragment() {
    ...
    private fun renderPlans(offers: List<PlanOffer>) {
        ...
        val config = com.edumio.app.remote.RemoteConfigProvider.get()
        val d = resources.displayMetrics.density
        val showYearly = config.getBoolean(com.edumio.app.remote.RemoteConfigKeys.PAYWALL_SHOW_YEARLY, false)

        // The value anchor is yearly-specific; only show it when yearly is actually offered.
        if (showYearly) {
            val anchor = config.getString(com.edumio.app.remote.RemoteConfigKeys.PREMIUM_VALUE_ANCHOR, "")
