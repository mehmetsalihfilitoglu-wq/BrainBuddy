...
import com.edumio.app.quiz.AnswerRecord
import com.edumio.app.quiz.Question
import com.edumio.app.quiz.QuizQualityPolicy
...
class RoomQuizDataStore(private val context: Context) {
    ...
    /** Candidate pool for fast quiz picking (LIMIT 2000, single difficulty). */
    ...
    fun getLgsCandidatePoolWithQuality(
        subject: String,
        allowedTiers: List<String> = QuizQualityPolicy.PLAYABLE_TIERS_PRIMARY,
    ): List<com.edumio.app.db.LgsCandidateRow> = runBlocking(Dispatchers.IO) {
    ...
    /** Seeding runs only from [com.edumio.app.app.EDUmioApp]; this reports whether meta says seeded. */
