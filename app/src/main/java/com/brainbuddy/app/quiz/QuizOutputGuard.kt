...
object QuizOutputGuard {
    ...
    data class Report(
        ...
        val repeatCount: Int,
        /** Questions rejected because options could not be made valid (no Seçenek fallback). */
    ...
    fun choicesInvalidReason(choices: List<String>, correctIndex: Int = -1): String? {
        ...
        // Case-sensitive comparison: AA/Aa/aa and capitalization-quiz options are distinct.
        val distinctTrimmed = choices.take(4).map { it.trim() }.distinct()
        if (distinctTrimmed.size < 4) return "DUPLICATE_CHOICES"

        // Quality gates that require knowing which option is correct.
     ...
     *
     * Conservative: uses the longest distractor (not the shortest) to avoid
     * false-positives on reading-comprehension questions where one long and
     * several short distractors are legitimate.
     */
    fun isHardTelegraphed(options: List<String>, correctIndex: Int): Boolean {
        ...
        // Guard: skip check for very short options (grade 1-2 numeric answers are fine)
     ...
     *
     * Only applied when min > 0 to avoid division by zero or sign-flip issues.
     */
    fun isWeakNumericDistractors(options: List<String>, correctIndex: Int = -1): Boolean {
        ...
        if (nums.size < 4) return false  // not all numeric — skip
        val min = nums.min()
        val max = nums.max()
        if (min <= 0) return false       // negative/zero values — skip ratio check
        return max / min > 2.5
    }
    ...
    private fun doSanitise(q: Question): Pair<Question, Boolean> {
        // STRICT QUALITY MODE: no recovery, no fixing.
        // A bad question that reaches here will be caught by the hard gate in validate().
        // DO NOT try to "repair" blank slots, DO NOT append v2/v3 suffixes to fake
        // distinct options. Both of those were recovery patterns that lowered quality.
        ...
        // Pad to exactly 4 slots with empty strings so validate() hard-gates them.
        ...
        // mutated=false: we changed nothing meaningful, caller logs are suppressed.
        return q.copy(presentationChoices = work) to false
    }
