#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate English3 packs and produce report. Compatible with QuestionPackImporter."""
import json
...
NEW_GEN_TYPES = {
    "dialogue_completion", "situation_response", "reading_comprehension",
    "situation_interpretation", "logical_response", "meaning_inference",
}


def main():
    packs = sorted(OUT_DIR.glob("lgs_eng3_pack_*.json"))
    total_questions = 0
    ...
    for p in packs:
        try:
            ...
            parse_ok = False
            continue
        if d.get("subject") != "ing":
            errors.append(f"{p.name}: expected subject=ing, got {d.get('subject')}")
    ...
    print("=" * 60)
    print("ENGLISH3 VALIDATION REPORT")
    print("=" * 60)
