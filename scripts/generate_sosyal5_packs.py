#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate 5th Grade Sosyal Bilgiler question bank for EDUmio.
50 packs × 10 questions = 500 questions.
EDUmio Question Design Standard: paragraf yorum, harita/tablo/grafik yorumlama, tarihsel analiz, sebep-sonuç.
"""
import json
import random
from pathlib import Path

OUT_DIR = Path(__file__).resolve().parent.parent / "app/src/main/assets/lgs_import/sosyal5"
...
TEMPLATES = [
    ...
    ("Toplumsal roller tablosunda \"öğrenci, öğretmen, veli\" rollerinin okul bağlamında birbirini tamamladığı belirtiliyor.\n\nBu bilgiye göre roller arası ilişki nasıldır?",
     "birey_toplum", ["Roller birbirini tamamlar ve toplumsal işlevi yerine getirir", "Her rol bağımsızdır", "Sadece öğretmen önemlidir", "Roller çakışır"]),
    # Kültür ve Miras
    ...
    ("\"Somut kültürel miras\" (anıtlar, müzeler) ile \"somut olmayan miras\" (masallar, türküler) ayrımı yapılıyor.\n\nHer iki miras türünün ortak işlevi nedir?",
     "kultur_miras", ["Toplumsal geçmişi ve kimliği geleceğe taşımak", "Sadece turizm için değerlidir", "Maddi değeri olan önemlidir", "Somut olmayan miras korunamaz"]),
    ("Tarih şeridinde \"yazının icadı\" önemli bir dönüm noktası olarak gösteriliyor.\n\nYazının icadının önemi aşağıdakilerden hangisidir?",
    ...
    ("Yerleşim tipleri (köy, kasaba, kent) metinde anlatılıyor. Kentlerde nüfus yoğunluğu ve iş kolları çeşitliliği daha fazladır.\n\nBu farkın sebebi nedir?",
     "insanlar_yerler_cevreler", ["Kentlerde sanayi, ticaret ve hizmetlerin yoğun olması", "Köyler daha büyüktür", "İklim farkı", "Sadece tarihsel nedenler"]),
    ...
    ("\"Bilim insanları gözlem ve deneyle doğayı anlamaya çalışır\" cümlesine göre bilimsel yöntemin temeli nedir?",
     "bilim_teknoloji_toplum", ["Gözlem, deney ve kanıta dayalı açıklama", "Sadece tahmin", "Kişisel görüş", "Gelenekler"]),
    ...
    ("Seçme ve seçilme hakkı metinde anlatılıyor. \"Bu hak demokratik yönetim için vazgeçilmezdir\" deniyor.\n\nBu hakkın kullanılması neden önemlidir?",
     "etkin_vatandaslik", ["Yönetimde söz sahibi olmak ve temsil için gereklidir", "Sadece belediye seçimlerinde geçerlidir", "Zorunlu değildir", "Oy kullanmak önemsizdir"]),
    # Küresel Bağlantılar
    ...
    ("Üretim faktörleri (toprak, emek, sermaye) tabloda veriliyor. Girişimci bunları bir araya getirir.\n\nGirişimcinin işlevi nedir?",
     "uretim_dagitim_tuketim", ["Kaynakları organize edip üretim sürecini başlatmak", "Sadece sermaye sağlar", "Girişimci gereksizdir", "Sadece işçi çalıştırır"]),
    ...
    for topic, count in counts.items():
        ...
        for _ in range(count):
            ...
            out.append(q(
                ...
                expl, f"sosyal5_{topic}_{len(out):04d}"
            ))
    return out
...
def main():
    ...
    for idx, qq in enumerate(all_q):
        qq["sourceRef"] = f"sosyal5_{idx:04d}"
        qq["id"] = f"sosyal5_{idx:04d}"
    ...
    for pi, questions in enumerate(packs):
        ...
        path = OUT_DIR / f"lgs_sosyal5_pack_{pi+1:03d}.json"
        with open(path, "w", encoding="utf-8") as f:
            json.dump(pack, f, ensure_ascii=False, indent=2)

    print("=" * 60)
    print("SOSYAL5 Question Bank Report")
    print("=" * 60)
