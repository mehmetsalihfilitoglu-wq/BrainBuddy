#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate Sosyal6 packs and produce report. Compatible with QuestionPackImporter."""
import json
...
NEW_GEN_TYPES = {"paragraf_yorum", "harita_yorumlama", "tablo_grafik_yorumlama", "tarihsel_olay_analizi", "cografi_yorum", "sebep_sonuc", "kavram_baglantisi"}

def main():
    packs = sorted(OUT_DIR.glob("lgs_sosyal6_pack_*.json"))
    total_questions = 0
    ...
    new_gen = sum(type_counts[t] for t in NEW_GEN_TYPES)
    new_gen_pct = (new_gen / total_questions * 100) if total_questions else 0

    print("=" * 60)
    print("SOSYAL6 VALIDATION REPORT")
    print("=" * 60)
    ...
    for d, c in sorted(diff_counts.items()):
        print(f"  {d}: {c}")
    print()
    print(f"New-generation types: {new_gen}/{total_questions} = {new_gen_pct:.1f}%")
    print()
