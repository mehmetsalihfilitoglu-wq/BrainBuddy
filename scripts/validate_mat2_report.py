#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate Mat2 packs and produce report. Compatible with QuestionPackImporter."""
import json
...
NEW_GEN_TYPES = {"gunluk_hayat_problemi", "cok_adinli_islem", "tablo_veri_yorumlama", "mantik_yuruttme", "yorum_gerektiren"}


def main():
    packs = sorted(OUT_DIR.glob("lgs_mat2_pack_*.json"))
    total_questions = 0
    ...
    for p in packs:
        try:
            ...
            parse_ok = False
            continue
        if d.get("subject") != "mat":
            errors.append(f"{p.name}: expected subject=mat, got {d.get('subject')}")
