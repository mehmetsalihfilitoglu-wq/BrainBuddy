#!/usr/bin/env node
/**
 * Build the EDUmio ORIGINAL question asset from content/edumio/.
 *
 * Source of truth: content/edumio/<subject>/*.json  (+ _calibration/*.json)  — original items.
 * Output (build artifact): app/src/main/assets/edumio/questions.json
 *
 * Kept fully SEPARATE from the official IMAT asset (app/src/main/assets/imat/imat_questions.json).
 * The app seeds these into Room as examType='EDUMIO', a pool isolated from official IMAT and LGS.
 *
 * Re-run whenever content/edumio changes:  node scripts/build_edumio_asset.js
 */
const fs = require('fs');
const path = require('path');

const REPO = path.resolve(__dirname, '..');
const SRC = path.join(REPO, 'content', 'edumio');
const OUT_DIR = path.join(REPO, 'app', 'src', 'main', 'assets', 'edumio');
