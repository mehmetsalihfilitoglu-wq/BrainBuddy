#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate Turkce2 packs and produce report. Compatible with QuestionPackImporter."""
import json
...
NEW_GEN_TYPES = {"kisa_metin_yorumlama", "gorsel_yorumlama", "cumlede_anlam", "sozcuk_anlami_cikarma", "ana_fikir", "metin_konusu"}


def main():
    packs = sorted(OUT_DIR.glob("lgs_turkce2_pack_*.json"))
    total_questions = 0
    ...
    for p in packs:
        try:
            ...
            parse_ok = False
            continue
        if d.get("subject") != "turkce":
            errors.append(f"{p.name}: expected subject=turkce, got {d.get('subject')}")
    ...
    print("=" * 60)
    print("TURKCE2 VALIDATION REPORT")
    print("=" * 60)
